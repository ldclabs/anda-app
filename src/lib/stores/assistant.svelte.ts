import type { ChatMessage, ChatSession } from '$lib/types/assistant'
...
class AssistantStore {
  ...
  async sendMessage(content: string) {
    ...
    try {
      ...
      const typingIndex = this.currentSession!.messages.findIndex(
        (m) => m.id === 'typing'
      )
      if (typingIndex !== -1) {
    ...
    if (index !== -1) {
      ...
      if (this._currentSessionId === sessionId) {
        this._currentSessionId =
          this._sessions.length > 0 ? this._sessions[0].id : null
      }
    }
  }
